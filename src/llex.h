...
enum RESERVED {
    ...
    TK_IMPORT,
    TK_DEFER,
    TK_ESCAPE, /* WARNING: if you add a new last terminal, make sure to update
                  NUM_RESERVED below to be the last terminal */
...
static inline void OutputBuffer_putc(OutputBuffer *buf, char c) {
    if (buf->N == buf->space) OutputBuffer_resize(buf, buf->space * 2);
    buf->data[buf->N] = c;
    buf->N++;
}
...
typedef struct LexState {
    ...
    int rethrow; /* set to 1 when le_luaexpr needs to re-report an error message, used to
                    suppress the duplicate addition of context information */
    char lextable; /* &lextable is the registry key for lua state associated with the
                      LexState object*/
} LexState;

LUAI_FUNC void luaX_init(terra_State *L);
LUAI_FUNC void luaX_setinput(terra_State *L, LexState *ls, ZIO *z, TString *source,
                             int firstchar);
