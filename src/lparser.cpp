...
static void simpleexp (LexState *ls, expdesc *v) {
  ...
  switch (ls->t.token) {
    case TK_NUMBER: {
      ...
      if(flags & ReadNumber::F_ISINTEGER) {
        push_integer(ls,ls->t.seminfo.i);
        const char * sign = (flags & ReadNumber::F_ISUNSIGNED) ? "u" : "";
        const char * sz = (flags & ReadNumber::F_IS8BYTES) ? "64" : "";
        char buf[128];
        ...
        if(flags & ReadNumber::F_IS8BYTES)
