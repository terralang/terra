...
static void closegoto (LexState *ls, int g, Labeldesc *label) {
  ...
  if (gt->nactvar < label->nactvar) {
    ...
    const char *msg = luaS_cstringf(ls->L,
...
static l_noret undefgoto (LexState *ls, Labeldesc *gt) {
  ...
  msg = luaS_cstringf(ls->L, msg, getstr(gt->name), gt->line);
  semerror(ls, msg);
}
#endif
...
Proto *luaF_newproto (lua_State *L) {
  Proto *f = (Proto*) malloc(sizeof(Proto)); //&luaC_newobj(L, LUA_TPROTO, sizeof(Proto), NULL, 0)->p;
  f->linedefined = 0;
  f->is_vararg = 0;
#if 0
  f->k = NULL;
...
static void constructor (LexState *ls, expdesc *t) {
  ...
  int pc = 0; //luaK_codeABC(fs, OP_NEWTABLE, 0, 0, 0);
...
static void assignment (LexState *ls, struct LHS_assign *lh, int nvars) {
  expdesc e;
  check_condition(ls, vkisvar(lh->v.k), "syntax error"); //TODO: audit, v.k is probably not set correctly
...
static void checkrepeated (FuncState *fs, Labellist *ll, TString *label) {
  ...
  for (i = fs->bl->firstlabel; i < ll->n; i++) {
    if (eqstr(label, ll->arr[i].name)) {
      const char *msg = luaS_cstringf(fs->ls->L,
...
static void exprstat (LexState *ls) {
  ...
  if (v.v.k == VCALL || (ls->t.token != '=' && ls->t.token != ','))  { /* stat -> func */
