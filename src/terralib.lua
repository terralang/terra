...
local function layoutstruct(st,tree,env)
    ...
    if st:iscomplete() then
        local msg = formaterror(tree,"attempting to define a completed struct")..formaterror(st:location(),"previous definition was here")
        error(msg,0)
    end
    ...
    st:definewithentries(getrecords(tree.records.entries),tree)
...
local function definedonce(st)
    local anchor = st:location()
    local defined = false
    function st:isdefined()
        return defined
    end
    function st:definewithentries(entries,tree)
        anchor = assert(tree)
        defined = true
        st:addentries(entries)
    end
    function st:location()
        return assert(anchor)
    end
    return st
end

local evaluateparameterlist,evaltype
local function evalformalparameters(env,tree,depth)
    ...
    diag:finishandabortiferrors("Errors reported during type evaluation.",depth)
    return r
end
local function evalfunctiontypeannotation(env,tree,depth)
    local diag = terra.newdiagnostics()
...
function terra.defineobjects(fmt,envfn,...)
    ...
    local function paccess(anchor,debugname,tbl,key,value)
        if type(tbl) ~= "table" then
            error(formaterror(anchor,"Expected a table before field '",key,"' but found ",terra.type(tbl)),4)
        end
...
function terra.anonfunction(tree,envfn)
    ...
    if tree.kind == "functiondecl" then -- function declaration
        local type = evalfunctiontypeannotation(env,tree.type,3)
        return T.terrafunction(nil,name,type,tree)
    else -- function definition
        tree = evalformalparameters(env,tree,3)
...
do
    ...
    local function memoizeproperty(erroronrecursion,getvalue)
        local cache = newweakkeytable()
        local inprogress = newweakkeytable()
        return function(self,onlywhencached)
            assert(onlywhencached == nil or type(onlywhencached) == "boolean", "onlywhencached should be a boolean")
            local v = cache[self]
            if not v then
                if onlywhencached then
                    return nil
                end
                if inprogress[self] then
                    erroratlocation(self:location(), erroronrecursion)
                end
                inprogress[self] = true
                v = getvalue(self)
                cache[self] = v
                inprogress[self] = nil
            end
    ...
    T.struct.getentries = memoizeproperty(
        ...
        function(self)
            ...
            local function checkentry(e,results)
               ...
               erroratlocation(self:location(),"expected either a field type pair (e.g. { field = <string>, type = <type> } or {<string>,<type>} ), or a list of valid entries representing a union")
            end
    ...
    local function reportopaque(type)
        ...
        if type.location then
            erroratlocation(type:location(),msg)
        else
            error(msg,4)
        end
    end
    T.struct.getlayout = memoizeproperty(
        "type recursively contains itself, or using a type whose layout failed",
        function(self)
            local tree = self:location()
...
function typecheck(topexp,luaenv,simultaneousdefinitions)
    ...
    function structcast(explicit,exp,typ, speculative)
        local from = exp.type:getlayout()
        local to = typ:getlayout()
...
local osterrapath = os.getenv("TERRA_PATH")
if osterrapath then
    package.terrapath = osterrapath:gsub(";;",";"..defaultpath..";")
end
