...
function T.quote:asvalue()
    local function getvalue(e)
        ...
        elseif e:is "operator" and (
                e.operator == tokens[">"] or
                e.operator == tokens[">="] or
                e.operator == tokens["<"] or
                e.operator == tokens["<="] or
                e.operator == tokens["+"] or
                ...
                e.operator == tokens["*"] or
                e.operator == tokens["/"] or
                e.operator == tokens["%"] ) and #e.operands == 2 then
            local op1 = getvalue(e.operands[1])
            local op2 = getvalue(e.operands[2])
            if op1 ~= nil and op2 ~= nil then
                if e.operator == tokens[">"] then return op1 > op2 end
                if e.operator == tokens[">="] then return op1 >= op2 end
                if e.operator == tokens["<"] then return op1 < op2 end
                if e.operator == tokens["<="] then return op1 <= op2 end
                if e.operator == tokens["+"] then return op1 + op2 end
        ...
        elseif e:is "operator" and (
                e.operator == "and" or
                e.operator == "or" ) and #e.operands == 2 then
            local op1 = getvalue(e.operands[1])
            local op2 = getvalue(e.operands[2])
            if op1 ~= nil and op2 ~= nil then
                if e.operator == "and" then return op1 and op2 end
                if e.operator == "or" then return op1 or op2 end
            end
            -- Short-circuit case
            if op1 ~= nil or op2 ~= nil then
                if e.operator == "and" and
                    ( (op1 ~= nil and not op1) or (op2 ~= nil and not op2) ) then
                    return false
                end
                if e.operator == "or" and (op1 or op2) then return true end
            end
        elseif e:is "operator" and e.operator == tokens["select"] and #e.operands == 3 then
            local cmp = getvalue(e.operands[1])
            if cmp ~= nil then
                if cmp then return getvalue(e.operands[2]) end
                return getvalue(e.operands[3])
            end
        end
