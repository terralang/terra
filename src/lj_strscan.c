...
#include <ctype.h>
#define lj_char_isdigit(c) isdigit(c)
#define lj_char_isspace(c) isspace(c)
#define LJ_CHAR_DIGIT	0x08
#define LJ_CHAR_XDIGIT	0x10

static int lj_char_isa(int c, char mask)
{
  switch(mask) {
  case LJ_CHAR_DIGIT: return isdigit(c);
  case LJ_CHAR_XDIGIT: return isxdigit(c);
...
static void strscan_double(uint64_t x, TLJ_TValue *o, int32_t ex2, int32_t neg)
{
  double n;
...
static StrScanFmt strscan_hex(const uint8_t *p, TLJ_TValue *o,
...
/* Parse octal number. */
static StrScanFmt strscan_oct(const uint8_t *p, TLJ_TValue *o,
...
static StrScanFmt strscan_dec(const uint8_t *p, TLJ_TValue *o,
			      StrScanFmt fmt, uint32_t opt,
